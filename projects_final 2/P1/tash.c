...
Description : This program implements a basic shell named tash, which can run both in interactive mode & batch mode.
			  ...
			  NOTE: We have modified the '<path to P1>' string to '/cs5348-xv6/src/jxm210003/P1/' so that
...
Functions:
	...
	3) readInput:  - Uses getline to read the input both in interactive mode and batch mode. 
			- It exits the program if we reach End Of File
...
char* readInput(){
	...
	if(numOfCharacters == -1){ //Exits if we reach End Of File
		exit(0);
	}
	else if(numOfCharacters < 0) { //If any error in accessing the input itself
		print_error();
	}
