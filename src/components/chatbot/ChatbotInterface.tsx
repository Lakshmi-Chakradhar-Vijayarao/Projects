...
export interface ChatbotInterfaceMessage {
  id: string;
  sender: 'user' | 'ai';
  text: string | React.ReactNode; // Allow JSX for AI messages if needed for formatting
  speakableText?: string; // Text for speech synthesis
}

interface ChatbotInterfaceProps {
  isOpen: boolean;
  onClose: () => void;
  messages: ChatbotInterfaceMessage[];
  currentInput: string;
...
const ChatbotInterface: React.FC<ChatbotInterfaceProps> = ({
  ...
  return (
    <div className="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-[9999] w-[calc(100%-3rem)] max-w-md sm:w-96">
      <div className="bg-card border border-border shadow-xl rounded-lg flex flex-col h-[70vh] max-h-[500px] sm:max-h-[600px]">
        ...
        <ScrollArea className="flex-grow p-3 sm:p-4 space-y-3">
          {messages.map((msg) => (
            ...
            >
              {msg.sender === 'ai' && <Bot className="h-5 w-5 flex-shrink-0" />}
